...
export class Buffer extends Uint8Array {
  static byteLength(v, encoding) { if (typeof v !== 'string') return v.byteLength ?? v.length; return encoding === 'latin1' || encoding === 'binary' ? v.length : enc.encode(v).length; }
  static from(v, encoding) {
    if (typeof v === 'string') {
      if (encoding === 'latin1' || encoding === 'binary') { const b = new Buffer(v.length); for (let i = 0; i < v.length; i++) b[i] = v.charCodeAt(i) & 0xff; return b; }
  ...
  toString(encoding = 'utf8', s = 0, e = this.length) {
    ...
    if (encoding === 'base64' || encoding === 'latin1' || encoding === 'binary') { let bin = ''; for (let i = 0; i < part.length; i += 0x8000) bin += String.fromCharCode.apply(null, part.subarray(i, i + 0x8000)); return encoding === 'base64' ? btoa(bin) : bin; }
