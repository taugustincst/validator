...
export function reportSheets(result, meta = {}) {
  ...
  const summary = [
    ...
    [],
    ['Roles in baseline', result.users.baseline], ['Roles in eCW exports', result.users.ecw], ['Roles in both', result.users.both],
    ...
    ['(role, setting) pairs compared', result.compared], ['… of which match', result.counts.ok],
    [],
    ...
    [],
    ['Sheets', ''],
    ['Actions', 'per role: what to remove, grant or review in eCW'],
    ['Findings', 'every discrepancy, worst first, with the row numbers in both files'],
    ['Side by side', 'every role × setting: expected vs. actual, colour-coded'],
    ['Roles / Settings', 'where the discrepancies concentrate'],
    ['Matches', 'settings and roles the validator paired by alias or by name rather than exactly'],
    ...(result.catalog ? [['Coverage', 'every catalog setting: its group, what it controls, whether the baseline covers it, how many eCW roles hold it'], ['Not in catalog', 'baseline (and eCW) setting names the catalog does not know — typos or renamed items, with the closest catalog name']] : []),
    [],
  ...
  const actions = [['Role', 'Mapped from', 'Remove in eCW (excess)', 'Grant in eCW (missing)', 'Review', 'Role status']];
  ...
  const findings = [
    ['Severity', 'Type', 'Role', 'Security setting', ...(hasCat ? ['Group', 'What it controls'] : []), 'Expected (baseline)', 'Actual (eCW)', 'Note', 'Baseline row', 'eCW row'],
  ...
  const users = [['Role', 'Mapped from', 'High', 'Medium', 'Low', 'Info', 'Excess', 'Missing', 'Different', 'Other'], ...perUser(result).map(u => [u.user, roleOf.get(u.user) || '', u.high, u.medium, u.low, u.info, u.excess, u.missing, u.different, u.other])];
  ...
  const coverage = hasCat ? [['Group', 'Security setting', 'What it controls', 'In baseline?', 'Granted in eCW to (roles)', 'Type'], ...[...result.catalog.settings].sort((a, b) => a.group.localeCompare(b.group) || a.name.localeCompare(b.name)).map(s => [s.group, s.name, s.description, s.inBaseline ? 'Y' : 'N', s.grantedTo, s.type])] : [];
  ...
  const matches = [['Kind', 'Baseline name', 'eCW name', 'Matched by'], ...result.matches.map(m => [m.kind === 'user' ? 'Role' : 'Setting', m.baseline, m.ecw, m.by === 'alias' ? 'alias you supplied' : 'same name, different category'])];
  ...
  return [
    ...
    { name: 'Roles', rows: users, widths: [26, 16, 8, 8, 8, 8, 8, 8, 10, 8], autofilter: true, styles: (r, c) => (r === 0 ? STYLE.header : c === 2 && users[r][2] > 0 ? STYLE.red : c === 3 && users[r][3] > 0 ? STYLE.amber : 0) },
...
export function textSummary(result, meta = {}) {
  ...
  L.push(`  roles: ${result.users.baseline} in baseline, ${result.users.ecw} in eCW, ${result.users.both} in both`);
  ...
  L.push(`  compared ${result.compared} role/setting pairs: ${result.counts.ok} match`);
  ...
  if (result.actions.length) {
    L.push(''); L.push('  What to do, per role:');
    let n = 0;
    for (const a of result.actions) {
      if (n++ >= limit) { L.push(`   … ${result.actions.length - limit} more roles (see the report)`); break; }
  ...
  if (low.length) { L.push(''); L.push(`  Coverage: ${low.length} setting/role pair(s) exist on only one side (low) — see the Findings sheet.`); const names = [...new Set(low.map(f => f.permission))]; L.push(`   ${names.slice(0, 6).join(' | ')}${names.length > 6 ? ' | …' : ''}`); }
  return L.join('\n');
}

const csvCell = v => { const s = String(v ?? ''); return /[",\n\r]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s; };
export function findingsCsv(result) {
  const rows = [['severity', 'type', 'role', 'setting', 'expected', 'actual', 'note', 'baseline_row', 'ecw_row'], ...result.findings.map(f => [f.severity, f.type, f.user, f.permission, f.expected, f.actual, f.note, f.baselineRow ?? '', f.actualRow ?? ''])];
